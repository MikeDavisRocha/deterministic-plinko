...
describe("the Seed Index", () => {
   ...
   * not have. 2 176 drops, and worth every millisecond.
   */
  it("puts every indexed seed in the bin it is filed under", () => {
    const board = outcomeFirstBoard();
    for (let bin = 0; bin < BIN_SEEDS.length; bin++) {
      for (const seed of BIN_SEEDS[bin]) {
  ...
  it("fills every bin's pool", () => {
    expect(BIN_SEEDS.length).toBe(BOARD.rows + 1);
    for (const pool of BIN_SEEDS) expect(pool.length).toBe(SEEDS_PER_BIN);
  });

  it("holds each seed once, in order", () => {
    for (const pool of BIN_SEEDS) {
  ...
  it("really is the first seeds, not merely some of them", () => {
    const PREFIX = 20_000;
    const board = outcomeFirstBoard();
    const found: number[][] = Array.from({ length: BIN_SEEDS.length }, () => []);
    for (let seed = 0; seed < PREFIX; seed++) {
      const bin = simulate(board, seed);
      if (bin >= 0) found[bin].push(seed);
    }
    for (let bin = 0; bin < BIN_SEEDS.length; bin++) {
      expect(BIN_SEEDS[bin].filter((s) => s < PREFIX)).toEqual(
        found[bin].slice(0, SEEDS_PER_BIN),
      );
    }
  });

  /**
   * The three drops that hang are excluded structurally rather than filtered
   * later: only a settled drop is ever pushed into a pool. A steered drop
   * therefore cannot draw a seed that never lands, which is the one way this
   * mode could have inherited that defect.
   */
  it("cannot hand out a seed that never settles", () => {
    const indexed = new Set(BIN_SEEDS.flat());
    for (const stuck of UNSETTLED_SEEDS) expect(indexed.has(stuck)).toBe(false);
  });
});

describe("steering to the Target Bin", () => {
  it("lands the drop where the commitment said it would", () => {
    const board = outcomeFirstBoard();
    for (let nonce = 0; nonce < 300; nonce++) {
      const drop = steerOf(seedsAt(nonce));
   ...
   */
  it("reproduces its committed drops", () => {
    expect([0, 1, 2, 186088, 146800].map((n) => {
      const d = steerOf(seedsAt(n));
  ...
  it("is a function of the seeds and nothing else", () => {
    expect(steerOf(seedsAt(11))).toEqual(steerOf(seedsAt(11)));
    expect(steerOf(seedsAt(11)).seed).not.toBe(steerOf(seedsAt(12)).seed);
  });
  ...
  it("draws its seed from inside the Target Bin's pool", () => {
    for (let nonce = 0; nonce < 300; nonce++) {
      ...
      expect(BIN_SEEDS[drop.targetBin][drop.poolIndex]).toBe(drop.seed);
    }
  });
  ...
  it("spreads across the pool rather than favouring its start", () => {
    ...
    for (let nonce = 0; nonce < 5_000; nonce++) {
      const drop = steerOf(seedsAt(nonce));
      if (drop.targetBin === BOARD.rows / 2) seen.add(drop.poolIndex);
    }
   ...
   * against exactly that. Exactly 64873/65536 — a rational, not a measurement,
