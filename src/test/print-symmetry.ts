 ...
 * deterministic map from that one number to a bin. Every board is exactly
 * mirror-symmetric (peg x are integers about an integer centerX — 350, 278,
 * 206), so the map ought to be antisymmetric: spawning at centerX - j must land
 * in bin (bins - 1) - b whenever spawning at centerX + j lands in bin b.
 *
 ...
 *
 *   npm run symmetry            -- 200 000 pairs per board
 *   npm run symmetry -- 20000   -- fewer, for a quick look
 */
...
for (const rows of ROW_COUNTS) {
  ...
  console.log(`\n${rows} rows — ${N} mirrored pairs from RNG jitters`);
  ...
  console.log(`${rows} rows — ${N} mirrored pairs on an even grid across the jitter range`);
...
// the distribution even with a perfectly symmetric solver. Board-independent —
// every board draws its jitter from the same stream — so it runs once.
let sum = 0;
let negatives = 0;
const M = 10_000_000;
